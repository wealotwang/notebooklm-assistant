...
### 🚀 Phase 3.10: 抢占式状态与双向触发 (Preemptive State & Dual Trigger)
**周期:** 2026-02-03 (v3.0.0.34)

#### 1. 抢占式状态捕获 (Preemptive State Capture)
*   **挑战**: 之前版本将 `setupAutoPinObserver` 放在了异步数据加载之后。由于 Gemini 页面加载速度极快，往往数据还没回来，URL 就已经被清洗（移除了 `usp=sharing`），导致观察者启动时扑了个空。
*   **修复**: 将状态捕获逻辑移至 `init()` 的**第一行同步执行**。脚本加载的瞬间，不管数据有没有准备好，先把 URL 里的关键参数锁死在 Session Storage 中。这确保了无论后续加载多慢，状态永远不会丢。

#### 2. UI 注入的双向兜底 (UI Injection Fallback)
*   **挑战**: v33 试图通过 Promise 链解决数据竞争，但导致了更严重的副作用——如果 Storage API 响应迟缓或 Promise 挂起，整个 UI 注入流程就会被无限期阻塞（白屏）。
*   **修复**: 采用“双向奔赴”策略。
    *   **路一**: `injectFolderUI` 立即启动轮询，不等待数据。先先把骨架搭起来。
    *   **路二**: 数据加载 `loadData` 并行执行。
    *   **汇合**: 无论哪一路先完成，都会尝试触发渲染。如果 UI 先好数据后到，数据回调会填充 UI；如果数据先好 UI 后到，UI 注入后的回调会读取数据。
