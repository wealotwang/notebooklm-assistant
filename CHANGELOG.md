...
## [3.0.0.34] - 2026-02-03
### Fixed
- **Gemini: Initialization Logic**: 修复了 v33 因数据加载 Promise 可能挂起而导致 UI 完全不注入的“白屏”问题。现在采用“抢占式状态捕获”与“双路 UI 触发”策略：
  - **同步捕获**: 脚本加载瞬间立即捕获 `usp=sharing` 状态，防止 URL 清洗导致判定失效。
  - **双向触发**: UI 注入不再死等数据加载，数据加载也不阻塞 UI 注入。两者谁先完成谁就尝试渲染，确保 UI 骨架始终可见，数据加载完成后自动填充。
...
- **Gemini: Render Stability**: 增强了数据渲染的健壮性。在 UI 注入后增加了强制渲染调用和详细日志输出 (`DOMService.log`)，确保在各种 SPA 路由切换场景下，文件夹和共享 Gem 列表都能立即显示内容，消除了“幽灵列表”现象。

## [3.0.0.30] - 2026-02-03
### Reverted
- **Gemini: Sidebar Auto-Hide**: 回退了 v27 引入的侧边栏自动隐藏逻辑（Auto-Hide）。原因是在某些宽屏或新标签页场景下，该逻辑会误判侧边栏状态，导致插件 UI 无法正常显示。现在插件 UI 将始终尝试显示，与 v3.0.0.26 行为一致。
...
- **Gemini: Sidebar Responsiveness**: 优化了侧边栏收起/展开的检测逻辑。现在插件能更敏锐地感知侧边栏宽度变化（如新标签页打开时），并在收起状态下（宽度<150px）自动隐藏插件 UI，避免布局错乱。
- **UI: Copy Update**: 将“文件夹分类”标题更新为 **“Chats 文件夹分类”**，使语义更精准。
